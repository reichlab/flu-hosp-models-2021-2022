...
weekly_data <- load_truth(hub = "FluSight") %>% 
...
#########################
submission <-paste0("~/GitHub/Flusight-forecast-data/data-experimental/",last_monday,"-UMass-trends_ensemble.csv")
...
for (i in 1:9) {
  ...
  p <- ggplot(exp_t, aes(fill=type_id, y=value, x=location)) + 
